"""High-level CGC AMX HD driver (HV-AMX-CTRL-4EDH)."""
...
from .amx_hd_base import AMXHDBase

class _AMXHDController(DllPortClaimRegistryMixin, TimeoutSafeDllMixin, AMXHDBase):
    """
    ...
    _EXPECTED_PRODUCT_TOKENS = ("AMX-CTRL-4EDH", "HV-AMX-CTRL-4EDH", "AMX")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(com=com, stream=stream, log=None, idn=device_id, dll_path=dll_path)
    ...
    def _warn_if_unexpected_product_id(self, timeout_s: Optional[float] = None):
        try:
            if timeout_s is None:
                status, product_id = self._call_locked(AMXHDBase.get_product_id, self)
            else:
                status, product_id = self._call_locked_with_timeout(
                    AMXHDBase.get_product_id,
    ...
    def connect(self, timeout_s: float = 5.0) -> bool:
        ...
        try:
            ...
            self.logger.info(
                ...
                f"on COM{self.com}, stream {self.stream}"
            )
    ...
    def _list_configs_unlocked(self, include_empty: bool = False) -> list[dict]:
        ...
        status, active_list, valid_list = AMXHDBase.get_config_list(self)
        ...
        for index, (active, valid) in enumerate(zip(active_list, valid_list)):
            if not include_empty and not (active or valid):
                continue
            name_status, name = AMXHDBase.get_config_name(self, index)
    ...
    def get_frequency_hz(self, timeout_s: Optional[float] = None) -> float:
        ...
        status, period = self._call_locked_with_timeout(
            AMXHDBase.get_oscillator_period,
            timeout_s,
        ...
        return self.DEF_CLOCK / (period + self.OSC_OFFSET)
    ...
    def set_frequency_hz(
        ...
        period = round((self.DEF_CLOCK / float(frequency_hz)) - self.OSC_OFFSET)
        ...
        timeout_s = self._resolve_io_timeout(timeout_s)
        status = self._call_locked_with_timeout(
            AMXHDBase.set_oscillator_period,
            timeout_s,
    ...
    def get_pulser_delay_seconds(
        ...
        return (delay + self.TIMER_DELAY_OFFSET) / self.DEF_CLOCK
    ...
    def get_pulser_width_seconds(
        ...
        return (width + self.TIMER_WIDTH_OFFSET) / self.DEF_CLOCK
    ...
    def _set_pulser_duty_cycle_unlocked(self, pulser_no: int, duty_cycle: float) -> None:
        ...
        status, period = AMXHDBase.get_oscillator_period(self, 0)
        self._raise_on_status(status, "get_oscillator_period")

        total_ticks = period + self.OSC_OFFSET
        width_register = round(total_ticks * duty_cycle - self.TIMER_WIDTH_OFFSET)
        if width_register < 1:
            raise ValueError(
        ...
        status = AMXHDBase.set_timer_width(self, pulser_no, width_register)
    ...
    def _get_product_info_unlocked(self) -> dict:
        product_no_status, product_no = AMXHDBase.get_product_no(self)
        self._raise_on_status(product_no_status, "get_product_no")
        product_id_status, product_id = AMXHDBase.get_product_id(self)
        self._raise_on_status(product_id_status, "get_product_id")
        fw_version_status, fw_version = AMXHDBase.get_fw_version(self)
    ...
    def _collect_housekeeping_unlocked(self) -> dict:
        # HD merges main/device/temperature state into one GetDeviceState call
        # and exposes controller bits via GetState.
        (
            state_status,
            main_state_hex,
            main_state_name,
        ...
        ) = AMXHDBase.get_housekeeping(self)
        self._raise_on_status(housekeeping_status, "get_housekeeping")
        oscillator_status, oscillator_period = AMXHDBase.get_oscillator_period(self, 0)
        self._raise_on_status(oscillator_status, "get_oscillator_period")

        timer_count_status, timer_count = AMXHDBase.get_timer_count(self)
        self._raise_on_status(timer_count_status, "get_timer_count")
        try:
            timer_count = max(0, int(timer_count))
        except (TypeError, ValueError):
            timer_count = 0

        pulsers = []
        for timer in range(timer_count):
            delay_status, delay_ticks = AMXHDBase.get_timer_delay(self, timer)
            self._raise_on_status(delay_status, f"get_timer_delay({timer})")
            width_status, width_ticks = AMXHDBase.get_timer_width(self, timer)
            self._raise_on_status(width_status, f"get_timer_width({timer})")
            burst = None
            try:
                burst_status, burst = AMXHDBase.get_timer_burst(self, timer)
                self._raise_on_status(burst_status, f"get_timer_burst({timer})")
            except Exception:
                burst = None
            pulsers.append(
                {
                    "pulser": timer,
                    "label": self.PULSER_LABELS.get(timer, str(timer)),
        ...
        oscillator_frequency_hz = self.DEF_CLOCK / (oscillator_period + self.OSC_OFFSET)
        return {
            "device_enabled": device_enabled,
            "main_state": {
                "hex": main_state_hex,
            ...
            "housekeeping": {
                "volt_12v_v": volt_12v,
                "volt_fans_v": volt_fans,
                "volt_5v0_v": volt_5v0,
                "volt_3v3_v": volt_3v3,
                "volt_3v3p_v": volt_3v3p,
                "volt_2v5p_v": volt_2v5p,
                "volt_vc_v": volt_vc,
                "temp_cpu_c": temp_cpu,
            },
            "oscillator": {
...
class AMXHD(ProcessIsolatedClientMixin):
    """Public AMX HD client with process isolation on Windows."""

    _INSTRUMENT_NAME = "AMX HD"
    _PROCESS_CONTROLLER_CLASS = _AMXHDController
    _PROCESS_CONTROLLER_PATH = f"{__name__}:_AMXHDController"
    _PROCESS_TIMEOUT_RULES = {
        "connect": (4.0, 5.0, 15.0),
        "initialize": (8.0, 5.0, 30.0),
    }
    _active_connections_lock = _AMXHDController._active_connections_lock
    _active_connections = _AMXHDController._active_connections
